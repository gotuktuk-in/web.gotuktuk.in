...
LiquidSlider.init = function (e, t) {
    ...
    n.build();
    n.events();
    if (!n.options.responsive && n.options.dynamicArrows)n.$sliderWrap.width(n.$sliderId.outerWidth(true) + n.leftArrow.outerWidth(true) + n.rightArrow.outerWidth(true));
    n.loaded = true;
...
LiquidSlider.build = function () {
    ...
    if (e.$sliderId.parent().attr("class") !== "ls-wrapper") {
        e.$sliderId.wrap('<div id="' + e.$elem.attr("id") + '-wrapper" class="ls-wrapper"></div>')
    }
    e.$sliderWrap = jQuery(e.sliderId + "-wrapper");
    e.options.preloader && e.addPreloader();
    jQuery(e.sliderId).children().addClass(e.$elem.attr("id") + "-panel panel");
    ...
    e.totalSliderWidth = e.$sliderId.outerWidth(true) + jQuery(e.$leftArrow).outerWidth(true) * t + jQuery(e.$rightArrow).outerWidth(true) * t;
    ...
    if (e.options.continuous) {
        e.$panelContainer.prepend(e.$panelContainer.children().last().clone().addClass("clone"));
        e.$panelContainer.append(e.$panelContainer.children().eq(1).clone().addClass("clone"))
    }
    ...
    e.slideDistance = e.options.responsive ? 100 : jQuery(e.sliderId).outerWidth();
...
LiquidSlider.determineAnimationType = function () {
    var e = this, t = "animation", n = "", r = "Webkit Moz O ms Khtml".split(" "), i = "", s = 0;
    e.useCSS = false;
    ...
    document.documentElement.clientWidth > e.options.useCSSMaxWidth && (e.useCSS = false)
};
...
LiquidSlider.transitionFade = function () {
    var e = this;
    jQuery(e.panelClass).eq(e.nextPanel).fadeTo(e.options.fadeInDuration, 1).css("z-index", 1);
    jQuery(e.panelClass).eq(e.prevPanel).fadeTo(e.options.fadeOutDuration, 0).css("z-index", 0);
    e.callback(e.options.callback, true)
};
LiquidSlider.hover = function () {
    var e = this;
    e.$sliderWrap.hover(function () {
        e.options.hoverArrows && e.hideShowArrows(e.options.fadeInDuration, true, true, false);
        e.options.pauseOnHover && clearTimeout(e.autoSlideTimeout)
    }, function () {
        e.options.hoverArrows && e.hideShowArrows(e.options.fadeOutnDuration, true, false, true);
...
LiquidSlider.setNextPanel = function (e) {
    ...
    if (t.loaded) {
        if (typeof e === "number") {
            ...
            t.options.continuous || (t.nextPanel = t.nextPanel < 0 ? t.panelCount - 1 : t.nextPanel % t.panelCount)
        }
...
LiquidSlider.getPanelNumber = function (e, t) {
    var n = this, r, i = e.replace("#", "").toLowerCase();
...
LiquidSlider.convertRegex = function (e) {
    return e.trim().replace(/[^\w -]+/g, "").replace(/ +/g, "-").toLowerCase()
};
LiquidSlider.updateClass = function () {
    var e = this, t = e.sanitizeNumber(e.nextPanel), n;
    if (e.options.dynamicTabs) {
        jQuery(e.$sliderWrap).find("> .ls-nav .tab" + e.sanitizeNumber(e.nextPanel)).addClass("current").siblings().removeClass("current")
    }
    if (e.options.crossLinks && e.crosslinks) {
        ...
        e.crosslinks.each(function () {
            ...
            if (n === "#" + e.getFromPanel(e.options.panelTitleSelector, t - 1) || n === "#" + t) {
                jQuery(this).addClass("currentCrossLink")
            }
        })
    }
    e.$panelClass.eq(e.sanitizeNumber(e.nextPanel) - 1).addClass("currentPanel").siblings().removeClass("currentPanel");
    e.$clones = jQuery(e.sliderId + " .clone");
    if (e.options.continuous && (e.nextPanel === -1 || e.nextPanel === e.panelCount)) {
...
LiquidSlider.finalize = function () {
    var e = this;
    var t = e.options.autoHeight ? e.getHeight() : e.getHeighestPanel(e.nextPanel);
...
LiquidSlider.callback = function (e, t) {
    ...
    if (e && n.loaded) {
        if (n.useCSS && typeof t !== "undefined") {
            jQuery(".panel-container").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function () {
                e.call(n)
            })
        } else {
            setTimeout(function () {
...
LiquidSlider.transitionOutAnimateCSS = function () {
    ...
    jQuery(e.panelClass).eq(e.prevPanel).addClass("animated " + e.options.animateOut);
...
LiquidSlider.transitionInAnimateCSS = function () {
    ...
    jQuery(e.panelClass).eq(e.nextPanel).addClass("animated " + e.options.animateIn);
...
LiquidSlider.transitionjQuery = function (e, t) {
    ...
    if (t) {
        ...
        n.panelContainer.animate({"margin-left": e + n.pSign}, {
...
LiquidSlider.getHeight = function (e) {
    var t = this;
    e = e || t.$panelClass.eq(t.sanitizeNumber(t.nextPanel) - 1).outerHeight(true);
...
LiquidSlider.addNavigation = function (e) {
    var t = this, n = "<" + t.options.navElementTag + ' class="ls-nav"><ul id="' + t.$elem.attr("id") + '-nav-ul"></ul></' + t.options.navElementTag + ">";
    ...
    if (t.options.mobileNavigation) {
        var r = t.options.mobileNavDefaultText ? '<option disabled="disabled" selected="selected">' + t.options.mobileNavDefaultText + "</option>" : null, i = '<div class="ls-select-box"><select id="' + t.$elem.attr("id") + '-nav-select" name="navigation">' + r + "</select></div>";
        ...
        jQuery.each(t.$elem.find(t.options.panelTitleSelector), function (e) {
            jQuery(t.$sliderWrap).find(".ls-select-box select").append('<option value="tab' + (e + 1) + '">' + jQuery(this).text() + "</option>")
        })
    }
    jQuery.each(t.$elem.find(t.options.panelTitleSelector), function (n) {
        jQuery(t.$sliderWrap).find(".ls-nav ul").append('<li class="tab' + (n + 1) + '"><a class="' + (e || "") + '" href="#' + (n + 1) + '">' + t.getNavInsides(this) + "</a></li>");
...
LiquidSlider.alignNavigation = function () {
    var e = this, t = e.options.dynamicArrowsGraphical ? "-arrow" : "";
    if (e.options.dynamicTabsAlign !== "center") {
        if (!e.options.responsive) {
            jQuery(e.$sliderWrap).find(".ls-nav ul").css("margin-" + e.options.dynamicTabsAlign, jQuery(e.$sliderWrap).find(".ls-nav-" + e.options.dynamicTabsAlign + t).outerWidth(true) + parseInt(e.$sliderId.css("margin-" + e.options.dynamicTabsAlign), 10))
        }
        jQuery(e.$sliderWrap).find(".ls-nav ul").css("float", e.options.dynamicTabsAlign)
    }
    e.totalNavWidth = jQuery(e.$sliderWrap).find(".ls-nav ul").outerWidth(true);
    if (e.options.dynamicTabsAlign === "center") {
        e.totalNavWidth = 0;
        jQuery(e.$sliderWrap).find(".ls-nav li a").each(function () {
...
LiquidSlider.registerNav = function () {
    ...
    e.$sliderWrap.find("[class^=ls-nav] li").on("click", function () {
        e.setNextPanel(parseInt(jQuery(this).attr("class").split("tab")[1], 10) - 1);
        return false
    })
};
LiquidSlider.addArrows = function (e) {
    var t = this, n = t.options.dynamicArrowsGraphical ? "-arrow " : " ";
    ...
    t.$sliderId.before('<div class="ls-nav-left' + n + (e || "") + '"><a href="#">' + t.options.dynamicArrowLeftText + "</a></div>");
    t.$sliderId.after('<div class="ls-nav-right' + n + (e || "") + '"><a href="#">' + t.options.dynamicArrowRightText + "</a></div>");
    t.leftArrow = jQuery(t.sliderId + "-wrapper [class^=ls-nav-left]").css("visibility", "hidden").addClass("ls-hidden");
    t.rightArrow = jQuery(t.sliderId + "-wrapper [class^=ls-nav-right]").css("visibility", "hidden").addClass("ls-hidden");
    if (!t.options.hoverArrows)t.hideShowArrows(undefined, true, true, false)
};
LiquidSlider.hideShowArrows = function (e, t, n, r) {
    var i = this, s = typeof e !== "undefined" ? e : i.options.fadeOutDuration, o = typeof e !== "undefined" ? e : i.options.fadeInDuration, u = t ? "visible" : "hidden";
    if (!n && (r || i.sanitizeNumber(i.nextPanel) === 1)) {
        ...
        i.leftArrow.stop().css("visibility", "visible").fadeTo(o, 1).removeClass("ls-hidden")
    }
    if (!n && (r || i.sanitizeNumber(i.nextPanel) === i.panelCount)) {
        ...
        i.rightArrow.stop().css("visibility", "visible").fadeTo(o, 1).removeClass("ls-hidden")
    }
};
...
LiquidSlider.adjustHeight = function (e, t, n, r) {
    ...
    i.$sliderId.animate({height: t + "px"}, {
        easing: jQuery.easing.hasOwnProperty(n || i.options.heightEaseFunction) ? n || i.options.heightEaseFunction : i.options.slideEaseFunctionFallback,
...
LiquidSlider.autoSlide = function () {
    ...
    if (e.options.autoSlideInterval < e.options.slideEaseDuration) {
        e.options.autoSlideInterval = e.options.slideEaseDuration > e.options.heightEaseDuration ? e.options.slideEaseDuration : e.options.heightEaseDuration
    }
    e.autoSlideTimeout = !document.hasFocus() ? undefined : setTimeout(function () {
...
LiquidSlider.registerCrossLinks = function () {
    var e = this;
    e.crosslinks = jQuery("[data-liquidslider-ref*=" + e.sliderId.split("#")[1] + "]");
    e.crosslinks.on("click", function (t) {
        e.options.autoSlide && e.startAutoSlide(true);
        e.setNextPanel(e.getPanelNumber(jQuery(this).attr("href").split("#")[1], e.options.panelTitleSelector));
        t.preventDefault()
    });
    e.updateClass()
};
LiquidSlider.updateHashTags = function () {
    var e = this, t = e.nextPanel === e.panelCount ? 0 : e.nextPanel;
...
LiquidSlider.makeResponsive = function () {
    var e = this;
    jQuery(e.sliderId + "-wrapper").addClass("ls-responsive").css({
        "max-width": jQuery(e.sliderId + " .panel:first-child").width(),
        width: "100%"
    });
    jQuery(e.sliderId + " .panel-container").css("width", 100 * e.panelCountTotal + e.pSign);
    jQuery(e.sliderId + " .panel").css("width", 100 / e.panelCountTotal + e.pSign);
...
LiquidSlider.responsiveEvents = function () {
    var e = this, t = e.options.hideArrowsThreshold || e.options.mobileUIThreshold || e.totalNavWidth + 10;
    if (e.$sliderId.outerWidth() < t) {
        ...
        if (e.options.dynamicArrows) {
            if (e.options.hideArrowsWhenMobile && (!e.leftArrow.length || !e.rightArrow.length)) {
                e.addArrows();
                e.registerArrows()
            } else if (!e.options.dynamicArrowsGraphical) {
                e.leftArrow.css("margin-" + e.options.dynamicTabsPosition, e.navigation.css("height"));
                e.rightArrow.css("margin-" + e.options.dynamicTabsPosition, e.navigation.css("height"))
            }
        }
    }
    jQuery(e.sliderId + "-wrapper").css("width", "100%");
    e.options.mobileNavigation && e.dropdownSelect.change(function () {
        e.setNextPanel(parseInt(jQuery(this).val().split("tab")[1], 10) - 1)
    })
};
LiquidSlider.registerTouch = function () {
    var e = this, t = e.options.swipeArgs || {
